...
namespace RVO
{
    ...
    {
        internal IList<KeyValuePair<float, Vector2>> agentNeighborsPosition_ = new List<KeyValuePair<float, Vector2>>();
        internal IList<KeyValuePair<float, Vector2>> agentNeighborsVelocity_ = new List<KeyValuePair<float, Vector2>>();
        ...
        internal void computeNeighbors()
        {
            ...
            //agentNeighborsPosition_.Clear();
            //agentNeighborsVelocity_.Clear();

            /*Vector2 posis = new Vector2(15, 2);
            float distSq = RVOMath.absSq(position_ - posis);
            agentNeighborsPosition_.Add(new KeyValuePair<float, Vector2>(distSq, posis));
            agentNeighborsVelocity_.Add(new KeyValuePair<float, Vector2>(distSq, new Vector2(0, 0)));*/
         ...
         */
        public abstract void interactWith(Vector2 pos, Vector2 veloc);
        ...
        */
        public void interactWithAgents()
        {
            ...
            for (int i = 0; i < agentNeighborsPosition_.Count; ++i)
            {
                //Agent neighbor = agentNeighbors_[i].Value;
                Vector2 neighborPosition = agentNeighborsPosition_[i].Value;
                Vector2 neighborVelocity = agentNeighborsVelocity_[i].Value;
                ...
                {*/
                //interactWith(neighbor);
                interactWith(neighborPosition, neighborVelocity);
                //}
            }
         ...
         */
        //internal void insertAgentNeighbor(Vector2 pos, Vector2 veloc, ref float rangeSq)
        internal void insertAgentNeighbor(Agent agent, ref float rangeSq)
        {
...

            {
                ...
                if (distSq < rangeSq)
                {
                    if (agentNeighborsPosition_.Count < maxNeighbors_)
                    {
                        agentNeighborsPosition_.Add(new KeyValuePair<float, Vector2>(distSq, agent.position_));
                        agentNeighborsVelocity_.Add(new KeyValuePair<float, Vector2>(distSq, agent.velocity_));
                    }

                    int i = agentNeighborsPosition_.Count - 1;

                    while (i != 0 && distSq < agentNeighborsPosition_[i - 1].Key)
                    {
                        agentNeighborsPosition_[i] = agentNeighborsPosition_[i - 1];
                        agentNeighborsVelocity_[i] = agentNeighborsVelocity_[i - 1];
                        --i;
                    }

                    agentNeighborsPosition_[i] = new KeyValuePair<float, Vector2>(distSq, agent.position_);
                    agentNeighborsVelocity_[i] = new KeyValuePair<float, Vector2>(distSq, agent.velocity_);

                    if (agentNeighborsPosition_.Count == maxNeighbors_)
                    {
                        rangeSq = agentNeighborsPosition_[agentNeighborsPosition_.Count - 1].Key;
                    }
                }
            }
        }
