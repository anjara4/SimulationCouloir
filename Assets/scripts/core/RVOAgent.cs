...
namespace RVO
{
    internal class RVOAgent : Agent
    {
        ...
        public override void computeNewVelocity()
        {
            ...
            if (agentNeighborsPosition_.Count == 0)
                neighboor = true;
        ...
        public override void interactWith(Vector2 pos, Vector2 veloc)
        {
            //SuperAgent other = agent;

            Vector2 relativePosition = pos - position_;
            Vector2 relativeVelocity = velocity_ - veloc;
            float distSq = (float)Math.Round(Vector2.absSq(relativePosition), 3);
            float combinedRadius = radius_ + 0.3f;
